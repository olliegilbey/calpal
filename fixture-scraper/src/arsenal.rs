...
use crate::parsing::DateTimeParser;
...
impl ArsenalScraper {
    ...
    fn parse_fixture_datetime(&self, date_str: &str, time_str: &str) -> Result<(DateTime<Utc>, crate::parsing::ParseMetadata), ScrapeError> {
        // Arsenal typically shows dates like "Sun Jul 27" and times like "12:30"
        // Use shared parsing utilities with London timezone and weekday tolerance
        let parser = DateTimeParser::new(London);
        
        match parser.parse_with_weekday_tolerance(date_str, time_str) {
            Ok((datetime, metadata)) => Ok((datetime, metadata)),
            Err(e) => Err(e),
        }
    }
    ...
    async fn scrape(&self) -> Result<Vec<ValidatedFixture>, ScrapeError> {
        ...
        let html_content = if self.use_browser && self.browser.is_some() {
            ...
            let response = self.client
                ...
                .send()
                .await
                .map_err(|e| ScrapeError::Network(format!("Failed to fetch Arsenal fixtures: {}", e)))?;
        ...
        for fixture_element in document.select(&fixture_selector) {
            ...
            if !datetime_str.is_empty() {
                match chrono::DateTime::parse_from_rfc3339(datetime_str) {
                    ...
                    Err(e) => {
                        eprintln!("Warning: Failed to parse ISO datetime '{}': {}", datetime_str, e);
                    }
                }
            } else {
...
mod tests {
    ...
    #[test]
    fn test_parse_fixture_datetime() {
        let scraper = ArsenalScraper::new();
        
        // Test summer fixture (BST - UTC+1)
        let result = scraper.parse_fixture_datetime("Sun Jul 27", "15:30");
        assert!(result.is_ok());
        
        let (datetime, metadata) = result.unwrap();
        // In July, London is BST (UTC+1), so 15:30 BST = 14:30 UTC
        assert_eq!(datetime.hour(), 14);
        assert_eq!(datetime.minute(), 30);
        assert!(metadata.original_source.contains("Sun Jul 27 15:30"));
    }

    #[test]
    fn test_parse_fixture_datetime_winter() {
        let scraper = ArsenalScraper::new();
        
        // Test winter fixture (GMT - UTC+0) - Wed Jan 15 is correct weekday for 2025
        let result = scraper.parse_fixture_datetime("Wed Jan 15", "15:30");
        assert!(result.is_ok());
        
        let (datetime, metadata) = result.unwrap();
        // In January, London is GMT (UTC+0), so 15:30 GMT = 15:30 UTC
        assert_eq!(datetime.hour(), 15);
        assert_eq!(datetime.minute(), 30);
        assert!(metadata.original_source.contains("Wed Jan 15 15:30"));
    }
    ...
    fn test_parse_invalid_datetime() {
        ...
        let result = scraper.parse_fixture_datetime("Invalid Date", "Invalid Time");
        assert!(result.is_err());
        
        match result {
            Err(ScrapeError::InvalidDateTime(_)) => (),
    ...
    async fn test_scrape_arsenal_fixtures_http() {
        ...
        match result {
            Ok(fixtures) => {
                println!("HTTP scraping succeeded: {} Arsenal fixtures", fixtures.len());
                for fixture in fixtures.iter().take(3) {
                    println!("  {} vs {} at {} on {}", 
                        fixture.fixture.team, fixture.fixture.opponent, fixture.fixture.venue, 
    ...
    async fn test_scrape_arsenal_fixtures_browser() {
        ...
        match scraper {
            Ok(scraper) => {
                ...
                match result {
                    Ok(fixtures) => {
                        println!("Browser scraping succeeded: {} Arsenal fixtures", fixtures.len());
                        for fixture in fixtures.iter().take(3) {
                            println!("  {} vs {} at {} on {}", 
                                fixture.fixture.team, fixture.fixture.opponent, fixture.fixture.venue, 
                                fixture.fixture.to_london_time().format("%Y-%m-%d %H:%M"));
                        }
                        assert!(!fixtures.is_empty(), "Should find fixtures with browser rendering");
                    }
                    Err(e) => {
            ...
            Err(e) => {
                println!("Browser initialization failed (Chrome/Chromium not available?): {}", e);
