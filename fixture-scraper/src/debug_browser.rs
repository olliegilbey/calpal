...
pub async fn debug_arsenal_html() -> Result<(), ScrapeError> {
    ...
    let html = browser.get_rendered_html("https://www.arsenal.com/fixtures").await?;
    ...
    println!("📊 Found {} .fixture-teaser elements (teasers)", fixture_count);
    ...
    println!("📊 Found {} .accordions containers", accordion_count);
    ...
    println!("📊 Found {} article elements inside accordions", article_count);
    
    // Check for /fixture/arsenal URLs
    let fixture_url_count = html.matches("/fixture/arsenal").count();
    println!("📊 Found {} '/fixture/arsenal' URLs", fixture_url_count);
    ...
    println!("📊 Found {} 'div.accordions article.card' elements (our target)", new_fixture_count);
